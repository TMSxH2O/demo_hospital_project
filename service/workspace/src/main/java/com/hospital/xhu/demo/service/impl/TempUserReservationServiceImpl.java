...
import com.hospital.xhu.demo.dao.impl.TempUserReservationMapperImpl;
 ...
 * 用户订单处理
 * 其中需要注意，查询出的订单信息虽然数据较多，但是部分字段并不在对应的数据库表中，
 * 因此，之后的更新等操作需要的并不能修改对应的字段，包括不能使用对应的字段进行查询
 *
 * 具体的字段，通过查看TempUserReservation类中的定义的属性即可。
 *
 * @author TMS_H2O
 * @version V1.0
 * @date 2021/5/10
...
@Service
@Slf4j
public class TempUserReservationServiceImpl implements ITempUserReservationService {

    private static final String CLASS_INFO_NAME = "预约订单 user_hospital_reservation";
    private final TempUserReservationMapperImpl userReservationMapper;

    public TempUserReservationServiceImpl(
            @Qualifier("tempUserReservationMapperImpl") TempUserReservationMapperImpl userReservationMapper) {
    ...
    public CommonResult<?> selectUserReservation(
        ...
        try {
            List<TempUserReservation> result =
     ...
     */
    @Override
    public CommonResult<?> insertUserReservation(List<TempUserReservation> userReservations) {
